...
## 创新点
1. 在自身个体发生交叉
2. 设计模块池，使用贝叶斯概率模型根据精英群体的模块排列特征选取每个基因，这里为了防止早熟可以设计两类群体，贝叶斯随机选择按照精英群体或者多样性群体的模块分布，多样性群体如何构建？
...
## 探索点
1. 2021-Multiobjective Evolutionary 使用了贝叶斯学习帕累托前沿组件的分布，只是计算了优秀组件的先后顺序排列，如何加入优秀的跳跃连接的学习？使用贝叶斯优化跳跃连接。使用贝叶斯学习优秀个体的连接方面的经验。所以我们研究的考虑了神经网络组件的排列顺序，跳跃连接，和权重三方面。并且每一层都有各自的组件池。同时我们的研究考虑了多场景。